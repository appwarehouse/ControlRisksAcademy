  ...
  useEffect(() => {
    if (studentId) {
      const fetchStudentDetails = async () => {
        const dbStudent = await studentService.details(studentId);
      ...
      fetchStudentDetails().catch(console.error);
    }
  }, [studentId]);
    ...
    if (!validateForm(e)) {
      if (studentId != null) {
        ...
        if (updated.success)
          ...
          toast.error(
            `There was an error updating student record. ${updated.message}`
          );
      } else {
        //create
        ...
        if (created.success) {
          ...
          toast.error(
            `There was an error creating student record. ${created.message}`
          );
        }
      }
    }
  ...
  return (
    <Container>
      <Col sm="12" className="mt-5" style={{ textAlign: "center" }}>
        <h3>
          Student Details {studentId && ` - ${studentDetails.studentNumber}`}
        </h3>
      </Col>
      <Col sm="12" md={{ size: 12 }} className="pb-5">
        <Form onSubmit={handleSubmit}>
          ...
          <FormGroup className="sm-6 mb-2 mr-sm-2 mb-sm-0 pb-2">
            ...
            <Input
              ...
              placeholder="Surname"
            />
            {errors.surname && <i style={{ color: "red" }}>{errors.surname}</i>}
          </FormGroup>
          <FormGroup className="sm-6 mb-2 mr-sm-2 mb-sm-0 pb-2">
          ...
          <FormGroup className="sm-6 mb-2 mr-sm-2 mb-sm-0 pb-2">
            ...
            <Input
              name="phoneNumber"
              value={
                studentDetails.phoneNumber === undefined
                  ? ""
